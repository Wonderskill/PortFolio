...
public class LoadManager : MonoBehaviour
{
    ...
    private void LoadingEnd()
    {
        ...
        // ARScene -> MainScene
        else if (ButtonManager.do_ar)
        {
            SceneManager.LoadScene("UIButton"); // ARScene로 이동
            ButtonManager.do_ar = false;        // 도착 완료
        }
        else if (MainSceneManager.back_main)
        {
            SceneManager.LoadScene("MainScene");  // MainScene로 이동
            MainSceneManager.back_main = false;    // 도착 완료
        }
    }
    #endregion
